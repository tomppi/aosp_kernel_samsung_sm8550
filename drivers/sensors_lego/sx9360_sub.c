...
#ifdef CONFIG_SENSORS_CHECK_PMIC_DEPENDENCY
...
static void sx9360_enter_error_mode(struct sx9360_p *data, enum grip_error_state err_state)
{
	...
	update_grip_error(1, data->err_state);
#endif
	GRIP_ERR("%d\n", data->err_state);
}
...
static int sx9360_get_data(struct sx9360_p *data)
{
	...
	u16 offset = 0;
	s16 avg = 0, diff = 0, retry = 0;
	u8 msByte, lsByte, convstat = 0;
...
#ifdef CONFIG_SENSORS_CHECK_PMIC_DEPENDENCY
static int sx9360_check_dependency(struct device *dev)
{
	...
	if (gpio_is_valid(upper_c2c_det)) {
		...
		if (gpio_level) {
			GRIP_INFO("upper-c2c-det-gpio %d(SMD)\n", upper_c2c_det);
			return 0;
		}
	}
#endif

	if (of_property_read_string_index(dNode, "sx9360_sub,dvdd_vreg_name", 0,
		(const char **)&dvdd_vreg_name)) {
		dvdd_vreg_name = NULL;

		GRIP_INFO("dvdd_vreg_name is null\n");
		return 0;
	}

	if (vdd_retry_cnt && !(vdd_retry_cnt % 50))
		GRIP_INFO("dvdd_vreg_name %s (%d)\n", dvdd_vreg_name, vdd_retry_cnt);
	...
	if (dvdd_vreg_name) {
		if (dvdd_vreg == NULL) {
			...
			GRIP_INFO("regulator_is_enabled %d\n", regulator_is_enabled(dvdd_vreg));
		}
	}
...
static int sx9360_parse_dt(struct sx9360_p *data, struct device *dev)
{
	...
	ret = of_property_read_u32(dNode, "sx9360_sub,unknown_sel", &data->unknown_sel);
	if (ret < 0) {
...
#ifdef CONFIG_SENSORS_CHECK_PMIC_DEPENDENCY
	ret = sx9360_check_dependency(&client->dev);
	if (ret < 0)
		return ret;
#endif
